...
## choose data path depending on whered fixed lens light or not
...
## chose type of shear pipeline, hypergalaxy etc
shear = 'pipeline_tag__with_shear__bd_align_centre/phase_4__lens_bulge_disk_sie__source_sersic/phase_tag__sub_2__pos_1.00/model.results'
#shear = 'pipeline_tag__hyper_galaxies__with_shear__bd_align_centre/phase_4__lens_bulge_disk_sie__source_sersic/phase_tag__sub_2__pos_1.00/model.results'
#shear = 'pipeline_tag__hyper_galaxies_bg_noise__with_shear__bd_align_centre/phase_4__lens_bulge_disk_sie__source_sersic/phase_tag__sub_2__pos_1.00/model.results'
...
for i in range(len(lens_name)):
    ...
    if full_data_path.is_file():
        data = pd.read_csv(full_data_path, sep='\s+', header=None, skiprows=59, nrows=9,).set_index(0)
        del data.index.name
...
for i in range(len(lens)):
    ...
    einstein_mass_error_low = lens_galaxy_low.einstein_mass_in_units(unit_mass='solMass', redshift_source=slacs['z_source'][i],
                                                       cosmology=cosmo)
    einstein_mass_test = lens_galaxy_test.einstein_mass_in_units(unit_mass='solMass',
    ...
    einstein_radii_rescaled = einstein_radius * 2 *(np.sqrt(results.loc[lens[i]]['param']['axis_ratio']))/ ((1+results.loc[lens[i]]['param']['axis_ratio']))
...
plt.axhline(y=0.02, color='k', linestyle='--')
plt.axhline(y=-0.02, color='k', linestyle='--')
plt.title('differince in radii (autolens with shear) as a function of axis ratio')
plt.xlabel('axis ratio', size=14)
plt.ylabel(r'$\frac{R_{E_{autolens}}-R_{E_{SLACS}}}{R_{E_{SLACS}}}$', size=14)
#plt.close()
