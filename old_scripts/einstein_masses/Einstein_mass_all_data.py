...
for i in range(len(lens_name)):
    full_data_path = data_path + lens_name[i] + '/pipeline_light_and_source_inversion/phase_3_both/model.results'
...
for i in range(len(results)):
    ...
    source_galaxy = al.Galaxy(light=al.lp.EllipticalSersic(centre=(results['source_light_centre_0'][i], results['source_light_centre_1'][i]),
                                                   axis_ratio=results['source_light_axis_ratio'][i], phi=results['source_light_phi'][i],
                                                   intensity=results['source_light_intensity'][i],
                                                   effective_radius=results['source_light_effective_radius'][i],
                                                   sersic_index=results['source_light_sersic_index'][i]),
...
ax.set_xlim(lims)
ax.set_ylim(lims)
#plt.savefig('/Users/dgmt59/Documents/Results/log_Einstein_mass_phase3.png', bbox_inches='tight', dpi=300)
plt.show()
