...
grid_stack = al.Grid.uniform(
                shape_2d=(100, 100), pixel_scales=0.05, sub_size=1)

g0 = al.Galaxy(redshift=0.5, mass_profile=al.mp.EllipticalIsothermal(
    centre=(0.0, 0.0), einstein_radius=1.4,axis_ratio=0.7, phi=40.0)
            )

plane = pl.plane = pl.AbstractGriddedPlane(
                galaxies=[g0],
...
magnification = plane.magnification(return_in_2d=True)
kappa_plane = plane.convergence_from_jacobian(return_in_2d=True)

# loading convergence, critical curves, and caustics
kappa = sie.convergence_from_grid(grid=grid, return_in_2d=True)
critical_curves = plane.tangential_critical_curve()
...
fig1 = plt.imshow(np.log10(kappa_plane), cmap='jet')
